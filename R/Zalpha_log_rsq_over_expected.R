...
#' \deqn{{Z_{\alpha}^{log_{10}(r^2/E[r^2])}}=\frac{{|L| \choose 2}^{-1}\sum_{i,j \in L}log_{10}(r^2_{i,j}/E[r^2_{i,j}]) + {|R| \choose 2}^{-1}\sum_{i,j \in L}log_{10}(r^2_{i,j}/E[r^2_{i,j}])}{2}}
...
#' @export
Zalpha_log_rsq_over_expected<-function(pos, x, cM, ws, LDprofile_cM_bins, LDprofile_rsq, minRandL = 4, minRL = 25, X = NULL){
