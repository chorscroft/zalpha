...
#' @param X Optional. Specify a region of the chromosome to calculate \eqn{Z_{\beta}^{E[r^2]}}{Zbeta} for in the format \code{c(startposition, endposition)}. The start position and the end position should be within the extremes of the positions given in the \code{pos} vector. If not supplied, the function will calculate \eqn{Z_{\beta}^{E[r^2]}}{Zbeta} for every SNP in the \code{pos} vector.
#'
...
Zbeta_expected<-function(pos, ws, dist, LDprofile_bins, LDprofile_rsq, minRandL = 4, minRL = 25, X = NULL) {
  ...
  outputList<-list(position=pos[pos>=X[1] & pos <= X[2]],Zbeta_expected=rep(NA,outputLength))
  ...
  for (i in 1:outputLength){
    ...
    if  (noL < minRandL || noR < minRandL || noL*noR < minRL){
      #NA
      outputList$Zbeta_expected[i]<-NA
    } else {
  ...
  if (sum(is.na(outputList$Zbeta_expected))==outputLength){
    warning("No Zbeta_expected values were calculated, try reducing minRandL and minRL or increasing the window size")
  }
  return(outputList)
}
