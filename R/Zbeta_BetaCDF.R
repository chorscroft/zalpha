...
#' @param X Optional. Specify a region of the chromosome to calculate \eqn{Z_{\beta}^{BetaCDF}}{Zbeta} for in the format \code{c(startposition, endposition)}. The start position and the end position should be within the extremes of the positions given in the \code{pos} vector. If not supplied, the function will calculate \eqn{Z_{\beta}^{BetaCDF}}{Zbeta} for every SNP in the \code{pos} vector.
#'
#' @return A list containing the SNP positions and the \eqn{Z_{\beta}^{BetaCDF}}{Zbeta} values for those SNPs
...
#' @export
Zbeta_BetaCDF<-function(pos, x, dist, ws, LDprofile_bins, LDprofile_Beta_a, LDprofile_Beta_b, minRandL = 4, minRL = 25, X = NULL){
  ...
  outputList<-list(position=pos[pos>=X[1] & pos <= X[2]],Zbeta_BetaCDF=rep(NA,outputLength))
  ...
  for (i in 1:outputLength){
    ...
    if  (noL < minRandL || noR < minRandL || noL*noR < minRL){
      ...
      rsqExp<-merge(data.frame(bins,rsq),data.frame(LDprofile_bins,LDprofile_Beta_a,LDprofile_Beta_b),by.x="bins",by.y="LDprofile_bins",all.x=TRUE,sort=FALSE)
      rsqSum<-sum(pbeta(rsqExp$rsq,rsqExp$LDprofile_Beta_a,rsqExp$LDprofile_Beta_b))

      outputList$Zbeta_BetaCDF[i]<-rsqSum/(noL*noR)
    }
  }
  if (sum(is.na(outputList$Zbeta_BetaCDF))==outputLength){
    warning("No Zbeta_BetaCDF values were calculated, try reducing minRandL and minRL or increasing the window size")
  }
  return(outputList)
}
