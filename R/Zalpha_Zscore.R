...
#'
#' @return A list containing the SNP positions and the \eqn{Z_{\alpha}^{Zscore}}{Zalpha} values for those SNPs
...
#' @export
Zalpha_Zscore<-function(pos, x, dist, ws, LDprofile_bins, LDprofile_rsq, LDprofile_sd, minRandL = 4, minRL = 25, X = NULL){
  ...
  for (i in 1:outputLength){
    ...
    if  (noL < minRandL || noR < minRandL || noL*noR < minRL){
      ...
      LrsqExp<-merge(data.frame(bins,Lrsq),data.frame(LDprofile_bins,LDprofile_rsq,LDprofile_sd),by.x="bins",by.y="LDprofile_bins",all.x=TRUE,sort=FALSE)
      ...
      RrsqExp<-merge(data.frame(bins,Rrsq),data.frame(LDprofile_bins,LDprofile_rsq,LDprofile_sd),by.x="bins",by.y="LDprofile_bins",all.x=TRUE,sort=FALSE)
