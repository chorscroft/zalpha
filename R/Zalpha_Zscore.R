...
#' @export
Zalpha_Zscore<-function(pos, x, cM, ws, LDprofile_cM_bins, LDprofile_rsq, LDprofile_sd, minRandL = 4, minRL = 25, X = NULL){
  ...
  #Check that the LDprofile vectors are the same length
  if (length(LDprofile_cM_bins) != length(LDprofile_sd)){
    stop("LDprofile_sd must contain the same number of values as there are bins given in LDprofile_cM_bins")
  }
  ...
  outputList<-list(position=pos[pos>=X[1] & pos <= X[2]],Zalpha_Zscore=rep(NA,outputLength))
  ...
  for (i in 1:outputLength){
    ...
    if  (noL < minRandL || noR < minRandL || noL*noR < minRL){
      ...
      LrsqExp<-merge(data.frame(bins,Lrsq),data.frame(LDprofile_cM_bins,LDprofile_rsq,LDprofile_sd),by.x="bins",by.y="LDprofile_cM_bins",all.x=TRUE,sort=FALSE)
      ...
      RrsqExp<-merge(data.frame(bins,Rrsq),data.frame(LDprofile_cM_bins,LDprofile_rsq,LDprofile_sd),by.x="bins",by.y="LDprofile_cM_bins",all.x=TRUE,sort=FALSE)
      RrsqSum<-sum((RrsqExp$Rrsq-RrsqExp$LDprofile_rsq)/RrsqExp$LDprofile_sd)

      outputList$Zalpha_Zscore[i]<-(LrsqSum/choose(noL,2)+RrsqSum/choose(noR,2))/2
    }
  }
  if (sum(is.na(outputList$Zalpha_Zscore))==outputLength){
    warning("No Zalpha_Zscore values were calculated, try reducing minRandL and minRL or increasing the window size")
  }
  return(outputList)
}
