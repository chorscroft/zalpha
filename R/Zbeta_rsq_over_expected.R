...
#' @param X Optional. Specify a region of the chromosome to calculate \eqn{Z_{\beta}^{r^2/E[r^2]}}{Zbeta} for in the format \code{c(startposition, endposition)}. The start position and the end position should be within the extremes of the positions given in the \code{pos} vector. If not supplied, the function will calculate \eqn{Z_{\beta}^{r^2/E[r^2]}}{Zbeta} for every SNP in the \code{pos} vector.
#'
#' @return A list containing the SNP positions and the \eqn{Z_{\beta}^{r^2/E[r^2]}}{Zbeta} values for those SNPs
...
#' Zbeta_rsq_over_expected(snps$positions,as.matrix(snps[,3:12]),snps$distances,3000,
...
#'
#' @export
Zbeta_rsq_over_expected<-function(pos, x, dist, ws, LDprofile_bins, LDprofile_rsq, minRandL = 4, minRL = 25, X = NULL){
  ...
  outputList<-list(position=pos[pos>=X[1] & pos <= X[2]],Zbeta_rsq_over_expected=rep(NA,outputLength))
  ...
  for (i in 1:outputLength){
    ...
    if  (noL < minRandL || noR < minRandL || noL*noR < minRL){
      #NA
      outputList$Zbeta_rsq_over_expected[i]<-NA
    } else {
      ...
      outputList$Zbeta_rsq_over_expected[i]<-rsqSum/(noL*noR)

      }
  }
  if (sum(is.na(outputList$Zbeta_rsq_over_expected))==outputLength){
    warning("No Zbeta_rsq_over_expected values were calculated, try reducing minRandL and minRL or increasing the window size")
  }
  return(outputList)
}
