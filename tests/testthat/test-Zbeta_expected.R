...
test_that("Zbeta_expected calculates Zbeta_expected statistic correctly", {

  expect_equal(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
               list(
...
test_that("Zbeta_expected calculates Zbeta_expected statistic correctly with a different window size", {

  expect_equal(Zbeta_expected(pos = df$POS, ws  = 1100, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
               list(
                 position=c(100,200,300,400,500,600,700,800,900,1000,1100,1200,1300,1400,1500),
...
test_that("Zbeta_expected calculates Zbeta_expected statistic correctly with X supplied", {

  expect_equal(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = c(700,900)),
               list(
...
test_that("Zbeta_expected fails with an X supplied outside of the region defined in pos", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = c(7000,9000)),
...
test_that("Zbeta_expected fails with an X supplied as a character", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = c("700bp","900bp")),
...
test_that("Zbeta_expected fails with an X supplied as only one number", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = 700),
               "X should be a numeric vector of length 2 e.g. c(100,200)",
               fixed=TRUE)
})
...
test_that("Zbeta_expected fails with an X supplied with too many numbers", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = c(700,900,1100)),
               "X should be a numeric vector of length 2 e.g. c(100,200)",
               fixed=TRUE)
})
...
test_that("Zbeta_expected fails when ws is non-numeric", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = "3000bp", dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
...
test_that("Zbeta_expected fails when ws is zero", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 0, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
               "ws must be a number greater than 0")
})
...
test_that("Zbeta_expected fails when pos is non-numeric", {
  ...
  expect_error(Zbeta_expected(pos = paste0(df$POS,"bp"), ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
...
test_that("Zbeta_expected fails when minLandR is non-numeric", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = "4snps", minRL = 25, X = NULL),
...
test_that("Zbeta_expected fails when minLandR is negative", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = -1, minRL = 25, X = NULL),
               "minRandL must be a number greater than or equal to 0")
})
...
test_that("Zbeta_expected fails when minLR is non-numeric", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = "25b", X = NULL),
...
test_that("Zbeta_expected fails when minLR is negative", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = -25, X = NULL),
               "minRL must be a number greater than or equal to 0")
})
...
test_that("Zbeta_expected warns about all NAs", {
  ...
  expect_warning(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 50, X = NULL),
...
test_that("Zbeta_expected fails when dist is non-numeric", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = paste0(df$dist,"dist"), LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
...
test_that("Zbeta_expected fails when dist is a different length to pos", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = c(df$dist,1), LDprofile_bins = LDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
...
test_that("Zbeta_expected fails when LDprofile_bins is non-numeric", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = paste0(LDprofile$bin,"dist"), LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
...
test_that("Zbeta_expected fails when LDprofile_bins are not of equal size", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = tempLDprofile$bin, LDprofile_rsq = LDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
...
test_that("Zbeta_expected fails when LDprofile_rsq is non-numeric", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = paste0(LDprofile$rsq,"r"), minRandL = 4, minRL = 25, X = NULL),
...
test_that("Zbeta_expected fails when LDprofile_rsq contains values not between 0 and 1", {
  ...
  expect_error(Zbeta_expected(pos = df$POS, ws  = 3000, dist = df$dist, LDprofile_bins = LDprofile$bin, LDprofile_rsq = tempLDprofile$rsq, minRandL = 4, minRL = 25, X = NULL),
